...
interface EcoLogNavbarProps {
  onBack: () => void
  onDashboard: () => void
  onChallenges: () => void
}

export function EcoLogNavbar({ onBack, onDashboard, onChallenges }: EcoLogNavbarProps) {
  return (
    <motion.header
    ...
    >
      <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <div className="hidden items-center gap-2 rounded-full bg-white/5 px-3 py-1.5 ring-1 ring-white/10 sm:flex">
          ...
          <span className="text-xs font-semibold text-zinc-200/80">Eco-Action Log</span>
        </div>
